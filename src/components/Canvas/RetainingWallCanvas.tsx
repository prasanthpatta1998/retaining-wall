  ...
  return (
    <main className="rw-canvas-card">
      ...
      <svg width={svgW} height={svgH} viewBox={`0 0 ${svgW} ${svgH}`} className="rw-svg">
        ...
        {/* Backfill */}
        <rect x={stemTR.x} y={Y(0)} width={X(draw.baseWidth) + 60 - stemTR.x} height={Y(draw.footTopY) - Y(0)} fill="url(#soil)" />
        <rect x={X(0) - 60} y={Y(draw.nglY)} width={stemBL.x - (X(0) - 60)} height={Y(draw.footTopY) - Y(draw.nglY)} fill="url(#soil)" />

        {/* Footing */}
        <rect x={X(0)} y={Y(draw.footTopY)} width={X(draw.baseWidth) - X(0)} height={Y(draw.footBotY) - Y(draw.footTopY)} fill="url(#concrete)" stroke={NAVY} strokeWidth={1.5} />

        {/* Stem */}
        <polygon points={`${stemBL.x},${stemBL.y} ${stemTL.x},${stemTL.y} ${stemTR.x},${stemTR.y} ${stemBR.x},${stemBR.y}`} fill="url(#concrete)" stroke={NAVY} strokeWidth={1.5} />

        {/* Shear Key */}
        {hasKey && <rect x={keyLeft} y={keyTop} width={keyRight - keyLeft} height={keyBot - keyTop} fill="url(#concrete)" stroke={NAVY} strokeWidth={1.5} />}

        {/* NGL Line */}
        <line x1={X(0) - 60} y1={Y(draw.nglY)} x2={X(draw.baseWidth) + 60} y2={Y(draw.nglY)} stroke="#1e293b" strokeDasharray="4,4" strokeWidth={1.2} />
        <polygon points={`${X(0) - 55},${Y(draw.nglY)} ${X(0) - 45},${Y(draw.nglY)} ${X(0) - 50},${Y(draw.nglY) + 8}`} fill="#1e293b" />
        <text x={X(0) - 62} y={Y(draw.nglY) - 6} textAnchor="end" fontSize={11} fontWeight={700} fill="#1e293b">NGL</text>

        {/* Reinforcement Drawings */}
        {stemVDots.map((p, i) => <circle key={`sv-${i}`} cx={p.x} cy={p.y} r={3} fill="#dc2626" />)}
        {stemVDots.length > 1 && <polyline points={stemVDots.map((p) => `${p.x},${p.y}`).join(" ")} fill="none" stroke="#dc2626" strokeWidth={1.2} />}
        {stemHLevels.map((p, i) => {
          const t = (p.y - stemBL.y) / (stemTL.y - stemBL.y || -1);
          return <line key={`sh-${i}`} x1={stemBL.x + (stemTL.x - stemBL.x) * t} y1={p.y} x2={stemBR.x + (stemTR.x - stemBR.x) * t} y2={p.y} stroke="#dc2626" strokeWidth={1} strokeDasharray="3,3" />;
        })}

        {footTopDots.map((p, i) => <circle key={`ft-${i}`} cx={p.x} cy={p.y} r={3} fill="#dc2626" />)}
        <line x1={footTopDots[0]?.x} y1={footTopDots[0]?.y} x2={footTopDots[footTopDots.length - 1]?.x} y2={footTopDots[0]?.y} stroke="#dc2626" strokeWidth={1.2} />
        {footBotDots.map((p, i) => <circle key={`fb-${i}`} cx={p.x} cy={p.y} r={3} fill="#dc2626" />)}
        <line x1={footBotDots[0]?.x} y1={footBotDots[0]?.y} x2={footBotDots[footBotDots.length - 1]?.x} y2={footBotDots[0]?.y} stroke="#dc2626" strokeWidth={1.2} />
        {keyVDots.map((p, i) => <circle key={`kv-${i}`} cx={p.x} cy={p.y} r={2.8} fill="#dc2626" />)}
        ...
        <HDim x1={X(0)} x2={X(draw.baseWidth)} y={Y(draw.keyBotY) + 55} label={`Base Width = x₁ + a₂ + x₂ = ${draw.x1} + ${draw.a2} + ${draw.x2} = ${draw.baseWidth}`} gap={-16} />
        ...
        <VDim y1={Y(0)} y2={Y(draw.keyBotY)} x={X(draw.baseWidth) + 90} label={`Overall Height = h + dof = ${draw.h + draw.dof}`} gap={0} />
        ...
        <line x1={stemTR.x + 18} y1={stemTR.y + 66} x2={(stemTL.x + stemTR.x) / 2} y2={stemTL.y + 55} stroke="#64748b" strokeWidth={1} />
