...
function Section({ title, variant = "blue", children }: SectionProps) {
  return (
    <div className="section">
      <h3 className={`section-title ${variant === "green" ? "section-title-green" : ""}`}>
        {title}
      </h3>
      {children}
    </div>
...
function HDim({ x1, x2, y, label, sub, gap = 26 }: HDimProps) {
  ...
  return (
    <g stroke={NAVY} strokeWidth={1} fill={NAVY}>
      ...
      <text x={(min + max) / 2} y={yy - 6} textAnchor="middle" fontSize={12} fontWeight={600} stroke="none" fill={NAVY}>
        {label}
      </text>
      {sub && (
        <text x={(min + max) / 2} y={yy - 20} textAnchor="middle" fontSize={10.5} stroke="none" fill={NAVY}>
          {sub}
        </text>
      )}
    </g>
...
function VDim({ y1, y2, x, label, gap = 26, anchor = "end", textGap = 8 }: VDimProps) {
  ...
  return (
    <g stroke={NAVY} strokeWidth={1} fill={NAVY}>
      ...
      <text x={tx} y={(min + max) / 2} textAnchor={anchor} fontSize={12} fontWeight={600} stroke="none" fill={NAVY}>
        {label}
      </text>
    </g>
  );
    ...
    <div className="rw-app">
      ...
      <div className="rw-panel">
        <Section title="Wall Geometry">
          ...
          <Field label="Depth of Foundation (dof)" value={geo.dof} onChange={setG("dof")} unit="mm" />
          <Field label="Width of the Toe (x₁)" value={geo.x1} onChange={setG("x1")} unit="mm" />
          <Field label="Width of the Heel (x₂)" value={geo.x2} onChange={setG("x2")} unit="mm" />
          <Field label="Depth of the Footing (d)" value={geo.d} onChange={setG("d")} unit="mm" />
          <Field label="Neglected Depth (nd)" value={geo.nd} onChange={setG("nd")} unit="mm" />
          <Field
            label="Provide Shear Key"
            value={geo.shearKey}
          ...
          {hasKey && (
            <>
              <Field label="Position of Shear Key from Toe (skp)" value={geo.skp} onChange={setG("skp")} unit="mm" />
        ...
        <h2 className="rw-heading">Reinforcement</h2>

        <Section title="Stem Reinforcement" variant="green">
          <Field label="Vertical Reinforcement - Dia" value={rebar.stemVDia} onChange={setR("stemVDia")} unit="mm" />
          <Field label="Vertical Reinforcement - Spacing (c/c)" value={rebar.stemVSpacing} onChange={setR("stemVSpacing")} unit="mm" />
          <Field label="Horizontal Reinforcement - Dia" value={rebar.stemHDia} onChange={setR("stemHDia")} unit="mm" />
          <Field label="Horizontal Reinforcement - Spacing (c/c)" value={rebar.stemHSpacing} onChange={setR("stemHSpacing")} unit="mm" />
        </Section>

        <Section title="Footing Reinforcement" variant="green">
          <Field label="Top Reinforcement - Dia" value={rebar.footTopDia} onChange={setR("footTopDia")} unit="mm" />
          <Field label="Top Reinforcement - Spacing (c/c)" value={rebar.footTopSpacing} onChange={setR("footTopSpacing")} unit="mm" />
          <Field label="Bottom Reinforcement - Dia" value={rebar.footBotDia} onChange={setR("footBotDia")} unit="mm" />
          <Field label="Bottom Reinforcement - Spacing (c/c)" value={rebar.footBotSpacing} onChange={setR("footBotSpacing")} unit="mm" />
        </Section>

        {hasKey && (
          <Section title="Shear Key Reinforcement" variant="green">
            <Field label="Vertical Reinforcement - Dia" value={rebar.keyVDia} onChange={setR("keyVDia")} unit="mm" />
            <Field label="Vertical Reinforcement - Spacing (c/c)" value={rebar.keyVSpacing} onChange={setR("keyVSpacing")} unit="mm" />
            <Field label="Horizontal Reinforcement - Dia" value={rebar.keyHDia} onChange={setR("keyHDia")} unit="mm" />
      ...
      <div className="rw-canvas">
        <svg width={svgW} height={svgH} viewBox={`0 0 ${svgW} ${svgH}`} className="rw-svg">
          <defs>
            ...
            <pattern id="soil" width={10} height={10} patternTransform="rotate(45)" patternUnits="userSpaceOnUse">
              <rect width={10} height={10} fill="#fbe6d4" />
              <line x1={0} y1={0} x2={0} y2={10} stroke="#d99a5b" strokeWidth={1.4} />
            </pattern>
            <pattern id="concrete" width={6} height={6} patternUnits="userSpaceOnUse">
              <rect width={6} height={6} fill="#eef0f2" />
              <circle cx={1.5} cy={1.5} r={0.5} fill="#c7ccd1" />
              <circle cx={4} cy={4} r={0.5} fill="#c7ccd1" />
            </pattern>
          </defs>
          ...
          <rect
            x={X(0) - 60}
            y={Y(draw.nglY)}
            width={X(0) - (X(0) - 60) + draw.x1 * draw.scale}
          ...
          />
          <polygon
            points={`${X(0) - 55},${Y(draw.nglY)} ${X(0) - 42},${Y(draw.nglY)} ${X(0) - 48.5},${Y(draw.nglY) + 10}`}
            fill="#1d6fb8"
          />
          <text x={X(0) - 62} y={Y(draw.nglY) - 6} textAnchor="end" fontSize={12} fontWeight={700} fill="#1d6fb8">
            NGL
          </text>

          {/* ---- reinforcement: stem ---- */}
          {stemVDots.map((p, i) => (
            <circle key={`sv-${i}`} cx={p.x} cy={p.y} r={3.2} fill="#d21f1f" />
          ))}
          {stemVDots.length > 1 && (
            <polyline points={stemVDots.map((p) => `${p.x},${p.y}`).join(" ")} fill="none" stroke="#d21f1f" strokeWidth={1.2} />
          )}
          {stemHLevels.map((p, i) => {
            ...
            return (
              <line key={`sh-${i}`} x1={leftX} y1={p.y} x2={rightX} y2={p.y} stroke="#d21f1f" strokeWidth={1} strokeDasharray="4,3" />
            );
          })}

          {/* ---- reinforcement: footing ---- */}
          {footTopDots.map((p, i) => (
            <circle key={`ft-${i}`} cx={p.x} cy={p.y} r={3.2} fill="#d21f1f" />
          ))}
          <line
            x1={footTopDots[0]?.x}
            y1={footTopDots[0]?.y}
          ...
          />
          {footBotDots.map((p, i) => (
            <circle key={`fb-${i}`} cx={p.x} cy={p.y} r={3.2} fill="#d21f1f" />
          ))}
          <line
            x1={footBotDots[0]?.x}
            y1={footBotDots[0]?.y}
          ...
          {keyVDots.map((p, i) => (
            <circle key={`kv-${i}`} cx={p.x} cy={p.y} r={3} fill="#d21f1f" />
          ))}
          ...
          <VDim y1={Y(0)} y2={Y(draw.footTopY)} x={X(0)} label={`Height of Backfill (h) = ${draw.h}`} gap={80} />
          ...
          {/* ---- callout labels ---- */}
          <text x={stemTR.x + 20} y={stemTR.y + 40} fontSize={12.5} fill="#b3261e" fontWeight={600}>
            {rebar.stemVDia}⌀ @ {rebar.stemVSpacing} c/c (Vertical)
          </text>
          <line x1={stemTR.x + 18} y1={stemTR.y + 36} x2={stemTL.x + 16} y2={stemTL.y + 30} stroke="#333" strokeWidth={1} />

          <text x={stemTR.x + 20} y={stemTR.y + 70} fontSize={12.5} fill="#b3261e" fontWeight={600}>
          ...
          <text x={X(draw.baseWidth) + 20} y={Y(draw.footTopY) + 6} fontSize={12.5} fill="#b3261e" fontWeight={600}>
            {rebar.footTopDia}⌀ @ {rebar.footTopSpacing} c/c (Top)
          </text>
          <text x={X(draw.baseWidth) + 20} y={Y(draw.footBotY) + 6} fontSize={12.5} fill="#b3261e" fontWeight={600}>
            {rebar.footBotDia}⌀ @ {rebar.footBotSpacing} c/c (Bottom)
          </text>

          {hasKey && (
            <text x={keyRight + 65} y={(keyTop + keyBot) / 2 + 30} fontSize={12} fill="#b3261e" fontWeight={600}>
              {rebar.keyVDia}⌀ @ {rebar.keyVSpacing} c/c (Key Vert.)
            </text>
          )}

          <text x={stemTR.x + 40} y={Y(0) - 12} fontSize={12.5} fontWeight={700} fill="#333">
            FILLED SOIL
          </text>
        </svg>
      </div>
